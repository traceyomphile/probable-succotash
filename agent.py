...
class RandomChessAgent(object):
	...
	def get_next_move(self, board: RawChessBoard, is_init=False): #must return src_row, src_col, dest_row, dest_col
		moves = _get_playable_moves(board, self.color, is_init, board.is_check(self.color))
		
		rnd_move = choice(_get_playable_moves(board, moves))
...
class NODE(object):
	def __init__(self, STATE: RawChessBoard, ACTIONS):
		self.STATE = STATE
		self.ACTIONS = ACTIONS
		self.WINS = 0		# Initially zero
		self.SIMULATIONS = 0	# Initially zero
		self.PARENT = None		# Intiially root node
		self.CHILDREN = []			# Initally empty

	def EXPAND(self):
		
		states_after_moves = [copy.deepcopy(self.STATE.get_state_after_move('black', a['src_row'], a['src_col'], a['dest_row'], a['dest_col'])) for a in self.ACTIONS]
		self.CHILDREN = [NODE(state, _get_playable_moves(state, 'black', self.PARENT is None, state.is_check('black'))) for state in states_after_moves]
